...
class TTSApp:
    ...
    def on_generate_button_click(self, b):
        ...
        with self.output_area:
            clear_output()
            display(widgets.HTML("<p style='font-size:16px; color:blue;'>正在生成语音，请稍候...</p>"))
        ...
        if not text:
            with self.output_area:
                clear_output()
                display(widgets.HTML("<p style='font-size:16px; color:red;'>请输入需要转换的文本内容</p>"))
            return
        ...
        try:
            ...
            with self.output_area:
                clear_output()
                display(widgets.HTML("<p style='font-size:16px; color:green;'>语音文件生成成功！您可以使用下方播放器进行播放。</p>"))
                display(widgets.HTML("<p style='font-size:14px;'>点击播放按钮来听听效果（最右侧按钮还能进行下载哦！）</p>"))
                display(Audio(self.audio_path))
        except subprocess.CalledProcessError as e:
            with self.output_area:
                clear_output()
                display(widgets.HTML(f"<p style='font-size:16px; color:red;'>语音文件生成失败：{e}</p>"))
