...
class StoryApp:
    def __init__(self):
        ...
        self.model_info = widgets.HTML(
            value="""
                <div style="font-size:18px; font-weight:bold; color:#000000; padding:10px 0; text-align:center;">
                    第1️⃣步：请从以上两个模型中点击一个开启神奇的AI之旅吧
                </div>
        ...
        # 标题标签
        self.story_type_label = widgets.Label(value="第2️⃣步：请先选择一个感兴趣故事类型吧（点击下拉菜单）:", style={'font_weight': 'bold', 'font_size': '16px'})
        self.background_label = widgets.Label(value="第3️⃣步：自定义你的故事背景:", style={'font_weight': 'bold', 'font_size': '16px'})
        self.continue_label = widgets.Label(value="第4️⃣步：现在就开始吧！书写专属于你的故事:", style={'font_weight': 'bold', 'font_size': '16px'})
        ...
        self.background_input = widgets.Text(
            placeholder='发挥你的想象力创造属于你的人物背景！当然如果你想使用默认背景，留空即可（选择好后点击“马上出发！”按钮以进入下一步）',
            layout=widgets.Layout(width='100%')
        )
    ...
    def on_continue_button_click(self, b):
        ...
        if user_text:
            ...
            if self.turn_count >= 6:
                ...
                # 显示结束图片
                with self.output_area:
                    display(widgets.HTML("<b style='font-size:24px; color: red;'>结束啦，感谢您的体验！您的支持就是我们更新的动力！</b>"))
    ...
    def on_exit_button_click(self, b):
        ...
        with self.output_area:
            clear_output()
            display(widgets.HTML("<b style='font-size:28px; color: red;'>已退出哦，感谢您的体验！您的支持就是我们更新的动力！</b>"))
            display(Image("story_app/image.jpg", width=300, height=200))
        self.continue_button.disabled = True
